...
if [ "$TYPE" == "RSA" ]; then
	...
	openssl req -x509 -new -nodes -key ca.key -sha256 -days 1825 -out ca.crt -subj "/C=CN/CN=UnblockNeteaseMusic Revive Root CA/O=$ISSUENAME"
	...
	openssl req -x509 -new -nodes -key ca.key -sha384 -days 1825 -out ca.crt -subj "/C=CN/CN=UnblockNeteaseMusic Revive Root CA/O=$ISSUENAME"
